...
export function detectFixTools() {
  ...
  if (IS_MAC) {
    try {
      // 安全：使用 execFileSync 避免命令注入
      execFileSync("brew", ["--version"], { stdio: "pipe", shell: IS_WIN });
      tools.hasBrew = true;
    } catch {}
  }
  ...
  if (IS_WIN) {
    try {
      execFileSync("winget", ["--version"], { stdio: "pipe", shell: true });
...
export async function upgradeNodeWithBrew(onProgress) {
  ...
  try {
    // 安全：使用 execFileSync 避免命令注入
    execFileSync("brew", ["upgrade", "node"], { stdio: "inherit", shell: IS_WIN, timeout: 120000 });
    onProgress(100, "Node.js 升级完成");
  } catch (err) {
...
export async function upgradeNodeWithWinget(onProgress) {
  ...
  try {
    // 安全：使用 execFileSync 避免命令注入
    execFileSync("winget", ["upgrade", "--id", "OpenJS.NodeJS", "--silent"], { stdio: "inherit", shell: true, timeout: 120000 });
    onProgress(100, "Node.js 升级完成");
  } catch (err) {
...
export async function installGit(tools, onProgress) {
  ...
  try {
    if (IS_MAC && tools.hasBrew) {
      execFileSync("brew", ["install", "git"], { stdio: "inherit", shell: IS_WIN, timeout: 120000 });
    } else if (IS_WIN && tools.hasWinget) {
      execFileSync("winget", ["install", "--id", "Git.Git", "--silent"], { stdio: "inherit", shell: true, timeout: 120000 });
    } else if (process.platform === "linux") {
      // Linux 需要 sudo，这里保留 shell 执行
      // 注意：这里的命令是固定的，没有用户输入，相对安全
      throw new Error("Linux 平台请手动执行: sudo apt-get install git 或 sudo yum install git");
    } else {
...
export async function installOpenclaw(onProgress) {
  ...
  try {
    // 安全：使用 execFileSync 避免命令注入
    execFileSync("npm", ["install", "-g", "openclaw@latest"], { stdio: "inherit", shell: IS_WIN, timeout: 120000 });
...
export async function fixOpenclawConfig(onProgress) {
  ...
  try {
    // 安全：使用 execFileSync 避免命令注入
    execFileSync("openclaw", ["doctor", "--fix"], { stdio: "inherit", shell: IS_WIN, timeout: 30000 });
