  ...
  try {
    ...
    try {
      ...
      if (/EACCES|permission denied/i.test(msg)) {
        console.log("Permission denied. Trying user-level install...");
        try {
          var userPrefix = join(homedir(), ".npm-global");
          execSync("npm config set prefix \"" + userPrefix + "\"", { stdio: "pipe", shell: true });
          execSync("npm install -g openclaw@latest", {
            stdio: "inherit",
            shell: true,
            env: {
          ...
          var userBin = IS_WIN ? userPrefix : join(userPrefix, "bin");
          if (process.env.PATH.indexOf(userBin) === -1) {
            process.env.PATH = userBin + (IS_WIN ? ";" : ":") + process.env.PATH;
          }

          console.log("OpenClaw installed successfully.");
          if (!IS_WIN) {
            console.log("Note: Add to your ~/.zshrc or ~/.bashrc:");
            console.log("  export PATH=\"" + userBin + ":$PATH\"");
          }
          return;
        } catch (userErr) {
          var userMsg = (userErr.stderr || userErr.message || "").toString();
          throw new Error(
            "Failed to install OpenClaw to user directory.\n" +
            (IS_WIN
              ? "Try running this program as Administrator."
              : "Please check:\n" +
                "  1. Network connection\n" +
                "  2. Disk space in home directory\n" +
                "  3. Or manually run: npm install -g openclaw@latest") +
            (userMsg ? "\n\nError: " + userMsg : "")
          );
        }
      }
