...
function getNodeVersionInfo(nodePath) {
  try {
    ...
    var output = execFileSync(nodePath, ["--version"], {
      stdio: "pipe",
      shell: IS_WIN // Windows 需要 shell 来解析 .cmd
    }).toString().trim();
...
function findNodePath() {
  ...
  try {
    // 安全：使用 execFileSync 避免命令注入
    var pathOut = execFileSync(IS_WIN ? "where" : "which", IS_WIN ? ["node"] : ["-a", "node"], {
      stdio: "pipe",
      shell: IS_WIN
    }).toString();
...
function ensureGit() {
  ...
  try {
    // 安全：使用 execFileSync 避免命令注入
    execFileSync("git", ["--version"], { stdio: "pipe", shell: IS_WIN });
    gitFound = true;
  } catch {}
  ...
  if (IS_WIN) {
    ...
    try {
      ...
      execFileSync("winget", [
        ...
        stdio: "inherit",
        shell: true, // Windows 需要 shell 来解析 winget
        timeout: 180000,
      });
...
export async function checkAndInstallOpenclaw() {
  ...
  try {
    // 安全：使用 execFileSync 避免命令注入
    execFileSync("openclaw", ["--version"], { stdio: "pipe", shell: IS_WIN });
    console.log("OpenClaw is already installed.");

    // 检查 Gateway 是否需要安装
    try {
      var doctorOutput = execFileSync("openclaw", ["doctor"], {
        stdio: "pipe",
        shell: IS_WIN,
        timeout: 10000
      }).toString();
      ...
      if (doctorOutput.includes("Gateway service not installed")) {
        ...
        try {
          execFileSync("openclaw", ["gateway", "install"], {
            stdio: "inherit",
            shell: IS_WIN,
            timeout: 60000
          });
    ...
    try {
      // 安全：使用 execFileSync 避免命令注入
      execFileSync("npm", ["install", "-g", "openclaw@latest"], {
        stdio: "inherit",
        shell: IS_WIN, // Windows 需要 shell 来解析 npm.cmd
        env: {
          ...process.env,
          GIT_CONFIG_COUNT: "1",
