...
import { diagnoseError } from "./error-diagnostics.js";
  ...
  const promise = new Promise(function(resolve, reject) {
    ...
    child.on("close", function(code) {
      if (code !== 0) {
        ...
        // 诊断错误
        const diagnosis = diagnoseError("Onboarding failed (exit " + code + ")", {
          profile: profile,
          logs: logs,
          stage: "onboarding"
        });
      ...
      try {
        ...
        const diagnosis = diagnoseError(errMsg, {
          profile: profile,
          logs: logs,
    ...
    child.on("error", function(err) {
      // 诊断错误
      const diagnosis = diagnoseError(err.message, {
        profile: profile,
        logs: logs,
        stage: "onboarding"
      });
