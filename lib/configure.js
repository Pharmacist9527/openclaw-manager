...
export const MODEL_CATALOG = [
  // Claude Opus
  { id: "claude-opus-4-6", name: "Claude Opus 4.6", provider: "evolink-anthropic" },
  { id: "claude-opus-4-5-20251101", name: "Claude Opus 4.5", provider: "evolink-anthropic" },
  { id: "claude-opus-4-1-20250805", name: "Claude Opus 4.1", provider: "evolink-anthropic" },
  // Claude Sonnet
  { id: "claude-sonnet-4-6", name: "Claude Sonnet 4.6", provider: "evolink-anthropic" },
  { id: "claude-sonnet-4-5-20250929", name: "Claude Sonnet 4.5", provider: "evolink-anthropic" },
  { id: "claude-sonnet-4-20250514", name: "Claude Sonnet 4", provider: "evolink-anthropic" },
  // Claude Haiku
  { id: "claude-haiku-4-5-20251001", name: "Claude Haiku 4.5", provider: "evolink-anthropic" },
  // Gemini
  { id: "gemini-3.1-pro-preview", name: "Gemini 3.1 Pro", provider: "evolink-google" },
  { id: "gemini-3-pro-preview", name: "Gemini 3.0 Pro", provider: "evolink-google" },
  { id: "gemini-3-flash-preview", name: "Gemini 3.0 Flash", provider: "evolink-google" },
  { id: "gemini-2.5-pro", name: "Gemini 2.5 Pro", provider: "evolink-google" },
  { id: "gemini-2.5-flash", name: "Gemini 2.5 Flash", provider: "evolink-google" },
  // GPT
  { id: "gpt-5.4", name: "GPT-5.4", provider: "evolink-openai" },
  { id: "gpt-5.2", name: "GPT-5.2", provider: "evolink-openai" },
  { id: "gpt-5.1", name: "GPT-5.1", provider: "evolink-openai" },
  { id: "gpt-5.1-chat", name: "GPT-5.1 Chat", provider: "evolink-openai" },
  { id: "gpt-5.1-thinking", name: "GPT-5.1 Thinking", provider: "evolink-openai" },
  // Doubao
  { id: "doubao-seed-2.0-pro", name: "Doubao Seed 2.0 Pro", provider: "evolink-openai" },
  { id: "doubao-seed-2.0-lite", name: "Doubao Seed 2.0 Lite", provider: "evolink-openai" },
  { id: "doubao-seed-2.0-mini", name: "Doubao Seed 2.0 Mini", provider: "evolink-openai" },
  { id: "doubao-seed-2.0-code", name: "Doubao Seed 2.0 Code", provider: "evolink-openai" },
  // Kimi
  { id: "kimi-k2-thinking", name: "Kimi K2 Thinking", provider: "evolink-openai" },
  { id: "kimi-k2-thinking-turbo", name: "Kimi K2 Thinking Turbo", provider: "evolink-openai" },
];
...
export function generateConfig(apiKey, modelId, channel, channelCreds, port) {
  ...
  const providers = {
    "evolink-anthropic": {
      ...
      apiKey: apiKey,
      models: MODEL_CATALOG.filter(m => m.provider === "evolink-anthropic").map(m => ({
        id: m.id,
        name: m.name,
      })),
    },
    "evolink-google": {
      ...
      apiKey: apiKey,
      models: MODEL_CATALOG.filter(m => m.provider === "evolink-google").map(m => ({
        id: m.id,
        name: m.name,
      })),
    },
    "evolink-openai": {
      ...
      apiKey: apiKey,
      models: MODEL_CATALOG.filter(m => m.provider === "evolink-openai").map(m => ({
        id: m.id,
        name: m.name,
      })),
    },
...
export function connectTelegramUser(profile, telegramId, onProgress) {
  ...
  if (onProgress) onProgress(65, "Installing gateway service...");
  try {
    execSafe("openclaw", [...profileArgs(profile), "gateway", "install"]);
  } catch (installErr) {
    if (onProgress) onProgress(65, "Warning: gateway install failed: " + installErr.message);
  }
...
// Ensure tools.exec config exists — patch old configs that predate exec approvals
function ensureExecConfig(profile) {
  const cfgPath = configPath(profile);
  try {
    const config = JSON.parse(readFileSync(cfgPath, "utf-8"));
    if (!config.tools) config.tools = {};
    if (!config.tools.exec || config.tools.exec.security !== "full" || config.tools.exec.ask !== "off") {
      config.tools.exec = { host: "gateway", security: "full", ask: "off" };
      writeFileSync(cfgPath, JSON.stringify(config, null, 2), "utf-8");
    }
  } catch (e) {
    // Config unreadable — skip patching
  }
}

export function startGateway(profile) {
  ensureExecConfig(profile);
  try {
    execSafe("openclaw", [...profileArgs(profile), "gateway", "install"]);
  } catch (e) {
    console.error("Warning: gateway install failed for profile " + profile + ":", e.message);
  }
...
export function changeModel(profile, modelId) {
  ...
  // Get API key from existing provider (prioritize evolink-* providers)
  let apiKey = config.models.providers[model.provider]?.apiKey
    || config.models.providers["evolink-anthropic"]?.apiKey
    || config.models.providers["evolink-google"]?.apiKey
    || config.models.providers["evolink-openai"]?.apiKey
    || config.models.providers.evolink?.apiKey
    || config.models.providers.anthropic?.apiKey;

  // Fallback: get any API key from any provider
  if (!apiKey) {
    for (const key in config.models.providers) {
      if (config.models.providers[key]?.apiKey) {
        apiKey = config.models.providers[key].apiKey;
        break;
      }
    }
  }
  ...
  // Add missing provider if needed
  if (!config.models.providers[model.provider]) {
    const providerConfig = {
      "evolink-anthropic": {
        api: "anthropic-messages",
        baseUrl: "https://direct.evolink.ai",
        apiKey: apiKey,
        models: MODEL_CATALOG.filter(m => m.provider === "evolink-anthropic").map(m => ({ id: m.id, name: m.name })),
      },
      "evolink-google": {
        api: "google-generative-ai",
        baseUrl: "https://direct.evolink.ai/v1beta",
        apiKey: apiKey,
        models: MODEL_CATALOG.filter(m => m.provider === "evolink-google").map(m => ({ id: m.id, name: m.name })),
      },
      "evolink-openai": {
        api: "openai-completions",
        baseUrl: "https://direct.evolink.ai/v1",
        apiKey: apiKey,
        models: MODEL_CATALOG.filter(m => m.provider === "evolink-openai").map(m => ({ id: m.id, name: m.name })),
      },
    };
    config.models.providers[model.provider] = providerConfig[model.provider];
  }
  ...
  config.agents.defaults.model.primary = model.provider + "/" + model.id;
...
export function listPairingRequests(profile, channel) {
  try {
    var out = execFileSync("openclaw", [...profileArgs(profile), "pairing", "list", channel, "--json"], {
      stdio: ["ignore", "pipe", "pipe"], timeout: 10000, shell: IS_WIN,
    }).toString();
    // openclaw may print log lines before JSON; extract the JSON object
    var jsonStart = out.indexOf("{");
    if (jsonStart < 0) return { requests: [], error: null };
    var parsed = JSON.parse(out.slice(jsonStart));
    return { requests: parsed.requests || [], error: null };
  } catch (e) {
    var stderr = (e.stderr || "").toString().trim();
    var msg = stderr || e.message || "unknown error";
    console.error("pairing list failed for " + profile + "/" + channel + ":", msg);
    return { requests: [], error: msg };
  }
}

export function approvePairing(profile, channel, code) {
  try {
    execFileSync("openclaw", [...profileArgs(profile), "pairing", "approve", channel, code], {
      stdio: ["ignore", "pipe", "pipe"], timeout: 15000, shell: IS_WIN,
    });
  } catch (e) {
    var stderr = (e.stderr || "").toString().trim();
    // If stderr mentions "already approved" or "not found", it's not a real failure
    if (/already|approved|not found|no pending/i.test(stderr)) return;
    throw new Error(stderr || e.message || "approve failed");
  }
}
