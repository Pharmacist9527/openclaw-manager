  ...
  const promise = new Promise(function(resolve, reject) {
    ...
    child.on("close", function(code) {
      ...
      if (code && code !== 0) {
        ...
        return reject({
          message: "Onboarding failed (exit " + code + ")",
          logs: logs
        });
      }
      ...
      try {
        ...
        // Force gateway config to be correct
      ...
      try {
        const preInstall = JSON.parse(readFileSync(cfgPath, "utf-8"));
        if (!preInstall.gateway) preInstall.gateway = {};
        preInstall.gateway.port = port;
        preInstall.gateway.mode = "local";
        writeFileSync(cfgPath, JSON.stringify(preInstall, null, 2), "utf-8");
      } catch (e) {
        onProgress(88, "Warning: failed to ensure gateway config: " + e.message);
      }
